...
│   ├── routers/
│   │   ├── enroll.py              # POST /enroll (handles mock/real ID-sync)
│   │   ├── users.py               # GET /users
...
│   │   └── crud.py                # user CRUD
│   └── storage/
│       └── audio_store.py         # upload validation + saving
├── dashboard/
│   ├── streamlit_app.py           # 2-tab dashboard (enrollment + analysis)
│   └── requirements.txt           # dashboard-only deps (streamlit, requests)
├── tests/
│   ├── test_risk_engine.py
...
├── requirements-real-ai.txt         # extra deps needed for AI_BACKEND=real
└── README.md
```

## 11. Project Status (Member 2 scope)

| Item | Status |
|---|---|
| FastAPI backend (`/enroll`, `/users`, `/analyze`) | ✅ Done, 21 automated tests passing |
| Context engine + risk fusion engine | ✅ Done, unit-tested, matches Section 3 formula exactly |
| SQLite persistence | ✅ Done |
| Streamlit dashboard (enrollment + "Speak Now" + call-simulation screens) | ✅ Done, verified against live backend |
| One-command dev launcher (`run_dev.py`) | ✅ Done, verified start + graceful shutdown |
| Integration with Member 1's real spoof/speaker models | ✅ Code complete — mock/real toggle, ID-sync bug fixed, missing AASIST files sourced. ⚠️ Not yet run with the real PyTorch/SpeechBrain stack end-to-end — run `check_real_ai_integration.py` on a machine with `requirements-real-ai.txt` installed to confirm before demo day |
